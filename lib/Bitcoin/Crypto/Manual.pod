...
trustless, decentralized system. Miner which found a block has the right to
...
To prove ownership of coins, Bitcoin uses assymetric cryptography over an
...
controls th private key.

=head3 Whitepaper and Bitcoin Improvement Proposals

Original Bitcoin was proposed and implemented by Satoshi Nakamoto. The original
document which defines how Bitcoin works (without much detail) is called the
Bitcoin Whitepaper. Since Bitcoin inception it has been modified heavily, and
most influential changes were introduced as Bitcoin Improvement Proposals
(I<BIPs>).
...
imported and turned into an extended key, which can be used to derive unlimited
number of addresses. Mnemonics can be stored securely away from devices
connected to the Internet, or without a computer at all, for example written of
a piece of paper - this is called I<cold storage>. Secrets stored on a computer
connected to an Internet are called I<hot storage>, and have greater than zero
chance to be hacked. If possible, cold storage should be used for storing large
sums, while hot storage can be used as "pocket money". Certain hardware wallets
exist, which are not connected to the internet, but can be connected to and
asked to sign a transaction.
...
Most packages in this module have types of their thrown exceptions
...
above. This mode will disallow creation of objects with other network than the
default. It can come handy if you are de-serializing keys from non-constant
...
=head3 Generate entropy from mnemonic deterministically
...
=head3 Create a Bitcoin script

L<Bitcoin::Crypto::Script> will help you build, de/serialize and run a script.
L<Bitcoin::Crypto::Script::Runner> gives you more control over script
execution, including running in the context of a certain transaction object, or
running the script step by step, allowing introspection after each opcode.
...
module in real-life scenarios. Keep L</DISCLAIMER> in mind.
...
as L<mempool.space>. UTXO data is necessary for performing many tasks on
...
Some unsolved problems are listed below. Contributions sorting out any of those
are welcome!
...
PSBT is a format for exchaning transaction data between parties in a
standardized way. Each PSBT consists of Global, Input and Output maps. Each map
consists of various fields which can be used to store data.
...
very powerful) machine has shown that up to legacy 600 transactions (each
...
Use the L<GitHub
bugtracker|https://github.com/Perl-Bitcoin/Bitcoin-Crypto/issues> to report
