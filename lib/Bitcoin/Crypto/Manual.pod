...
exist, which are not connected to the Internet, but can be connected to via a
private channel and asked to sign a transaction.
...
very powerful) machine has shown that up to 600 legacy transactions (each
