...
=item * serializing bitcoin blocks
...
very powerful) machine has shown that up to 500 transactions (each having 2
P2MS inputs) can be decoded and verified per second, per process.
