...
subtest 'should serialize transactions' => sub {
	...
	$tx->add_output(
		...
		locking_script => Bitcoin::Crypto::Script->new
			->add('OP_DUP')
			->add('OP_HASH160')
			->push(pack 'H*', 'cf0d26e32df5b94905a7f372e4db12132be29f8e')
			->add('OP_EQUALVERIFY')
			->add('OP_CHECKSIG')
	);
	...
	$tx->add_output(
		...
		locking_script => Bitcoin::Crypto::Script->new
			->push(chr 0)
			->push(pack 'H*', '28487e88a2870efdd700526a8904cfd78293a678')
	);

	$tx->add_witness(
		pack('H*', '304402201c57d633dea588b7c7e5b42e3fd72b7131c154293032dde60e380844403c2402022075a9eb1690de8b2dde0d45d643bcb64ad0c1782ca4cf1a9ca37c0b92e705013301'),
		pack('H*', '023a95ab5d95fd2ca4a849e66124e55a549a6e7573dfed0b7356f74ac3862f3901')
	);
